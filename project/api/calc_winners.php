...
$query = "SELECT * from competitions c WHERE expires <= current_timestamp() and calced_winner != 0 and participants >= min_participants";
$db = getDB();
...
if ($r) {
    ...
    if ($comps) {
        ...
        foreach ($comps as $c) {
            ...
            $cid = safe_get($c, "id", -1);
            $query = "SELECT s.user_id, SUM(s.score) as total from scores s JOIN usercompetitions uc on s.user_id = uc.user_id WHERE s.created BETWEEN '2021-05-08 00:00:00' AND '2021-05-09 00:00:00' AND uc.competition_id = 4 group by user_id order by total desc limit 3";
            $stmt = $db->prepare($query);
            ...
            if ($r) {
                ...
                $query = "UPDATE competitions set calced_winner = 1 where id = id";
                $stmt = $db->prepare($query);
...
$query = "UPDATE competitions set calced_winner = 1 WHERE expires <= current_timestamp() and calced_winner != 1 and participants < min_participants";
$db = getDB();
$stmt = $db->prepare($query);
$r = $stmt->execute();
$r = $stmt->rowCount();
